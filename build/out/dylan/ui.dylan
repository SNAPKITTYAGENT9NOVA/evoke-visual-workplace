...
define generic on-dylan-event
    (wp :: <visual-workplace>, event :: <dylan-event>) => ();
...
define method open-file!
  ...
  ed.editor-path := path;
  // Buffer would be loaded from the project tree in a real Open Dylan app.
...
define method handle-key-enter!
  ...
  elseif (subsequence-position(line, "gcc")
    ...
    let src = if (subsequence-position(line, ".c"))
                // naive: last token
                line
              else
                "hello.c"
...
define method on-dylan-event
    (wp :: <visual-workplace>, event :: <dylan-event>) => ()
  let wb = wp.workplace-workbench;
  select (event.event-kind)
    #"open-file" =>
      open-file!(wb, as(<byte-string>, event.event-payload | ""));
    #"compile-c" =>
      if (wb.workbench-termux & wb.workbench-agent)
        compile-c!(wb.workbench-termux, wb.workbench-agent,
                   as(<byte-string>, event.event-payload | "hello.c"));
      end if;
    #"glow-on" =>
      if (wb.workbench-termux)
        set-glow!(wb.workbench-termux.termux-glow, #t);
      end if;
    #"glow-off" =>
      if (wb.workbench-termux)
        set-glow!(wb.workbench-termux.termux-glow, #f);
      end if;
    #"key-enter" =>
      if (wb.workbench-termux & wb.workbench-agent)
        handle-key-enter!(wb.workbench-termux, wb.workbench-agent,
                          as(<byte-string>, event.event-payload | ""));
      end if;
    #"agent-ask" =>
      if (wb.workbench-agent)
        agent-ask!(wb.workbench-agent,
                   as(<byte-string>, event.event-payload | ""));
      end if;
    #"syscall-trace" =>
      if (wb.workbench-agent)
        agent-post!(wb.workbench-agent, "syscall",
                    as(<byte-string>, event.event-payload | ""));
      end if;
    otherwise => #f;
  end select;
end method on-dylan-event;
